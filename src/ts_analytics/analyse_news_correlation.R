...
news.count.filled <- ddply(news.count.df, .(cnpj), function(df){
...
d_ply(emp.ts.filled, .(cnpj, cod_isin), function(df){
    ...
    news.df <- subset(news.count.df, cnpj == ts.cnpj)
    news.ts <- zoo(news.df$num_noticias, news.df$data_noticia)
    
    news.df.filled <- subset(news.count.filled, cnpj == ts.cnpj)
    ...
    PlotTsWithSolavanco(intersect.price.ts, solavancos, 
                        ylab = "Preco_Ultimo",
                        main.title = paste("Time-Series with Solavancos (", ts.nome.pregao, 
                                           " - ", ts.cod_isin, 
