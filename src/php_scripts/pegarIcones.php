...
$quebraSite = split($padrao, $sitePrincipal);//quebra a string(parametro 2) pelo padrao(parametro 1) deixando as partes em um array
$codigoParteIcones = $quebraSite[2];//pega a listagem por ordem alfabetica
$empresas = split("<li>", $codigoParteIcones);//quebrando em <li> cada elemento do array sera o codigo de cada empresa, exceto pelo elemento 0 que nao eh empresa
...
for($i = 1; $i <= count($empresas); $i++){
	...
	$href = split(" ", $empresa);//quebra nos espacos em branco, para pegar cada parte
	$site = split("\"", $href[1]);//href[1] eh a parte href="/3m", sendo /3m o complemento do site para pegar a logo dessa empresa
	
	$empresaSemEspacos = trim($empresa);//eliminar espacos, tabulacoes... no inicio e final da string
	$linhaTitle = split("title=", $empresaSemEspacos);//nessa parte title fica o nome de pregao da empresa
	$title = split("\"",$linhaTitle[1]);//o elemento $linhaTitle[1] tem tudo depois de title=", como o nome da empresa so vai ate a proxima aspa, entao quebra-se a string na aspa
	...
	$figColor = split("figColor", $siteImagem);//id da imagem da logomarca eh figColor
	$codigoImagem = split("<", $figColor[1]);
	$quebraLink = split("src=", $codigoImagem[1]);//depois do src= eh o complemento do link da logomarca
